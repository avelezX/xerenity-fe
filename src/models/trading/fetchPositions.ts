...
export const fetchXccyPositions = async (
  ...
  try {
    const { data, error } = await supabase
      .schema(SCHEMA)
      .rpc('get_xccy_positions', { p_company_id: companyId ?? null });
    if (error) {
...
export const fetchNdfPositions = async (
  ...
  try {
    const { data, error } = await supabase
      .schema(SCHEMA)
      .rpc('get_ndf_positions', { p_company_id: companyId ?? null });
    if (error) {
...
export const fetchIbrSwapPositions = async (
  ...
  try {
    const { data, error } = await supabase
      .schema(SCHEMA)
      .rpc('get_ibr_swap_positions', { p_company_id: companyId ?? null });
    if (error) {
...
export const fetchTesPositions = async (
  ...
  try {
    const { data, error } = await supabase
      .schema(SCHEMA)
      .rpc('get_tes_positions', { p_company_id: companyId ?? null });
    if (error) {
